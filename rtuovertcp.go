...
type rtuovertcpPackager struct {
...
//  Data: n bytes
func (mb *rtuovertcpPackager) Encode(pdu *ProtocolDataUnit) (adu []byte, err error) {
...
func (mb *rtuovertcpPackager) Verify(aduRequest []byte, aduResponse []byte) (err error) {
...
//  Length: 2 bytes
//  Unit identifier: 1 byte
func (mb *rtuovertcpPackager) Decode(adu []byte) (pdu *ProtocolDataUnit, err error) {
	length := len(adu)
	// Calculate checksum
	var crc crc
	crc.reset().pushBytes(adu[0 : length-2])
...
func (mb *rtuovertcpTransporter) calculateDelay(chars int) time.Duration {
	return time.Duration(750*chars+1750) * time.Microsecond
}
...
func (mb *rtuovertcpTransporter) Connect() error {
	mb.mu.Lock()
	defer mb.mu.Unlock()
...
func (mb *rtuovertcpTransporter) startCloseTimer() {
	if mb.IdleTimeout <= 0 {
		return
	}
...
func (mb *rtuovertcpTransporter) Close() error {
	mb.mu.Lock()
	defer mb.mu.Unlock()
...
func (mb *rtuovertcpTransporter) flush(b []byte) (err error) {
...
func (mb *rtuovertcpTransporter) logf(format string, v ...interface{}) {
	if mb.Logger != nil {
...
func (mb *rtuovertcpTransporter) close() (err error) {
	if mb.conn != nil {
		err = mb.conn.Close()
		mb.conn = nil
	}
...
func (mb *rtuovertcpTransporter) closeIdle() {
	mb.mu.Lock()
	defer mb.mu.Unlock()
